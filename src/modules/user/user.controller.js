...
export const getAllusers =catchError( async (req, res, next) => {
  let apiFeatures = new APiFeatures(userModel.find({}).select('-password').populate("posts"),req.query).pagination().sort().search().fields()
...
export const getUserById = catchError(async (req, res, next) => {
  let { id } = req.params;
  let Foundeduser = await userModel.findById(id).populate("posts").select("-password");
  !Foundeduser && next(new AppError("user not found", 404))
  if (Foundeduser) {
